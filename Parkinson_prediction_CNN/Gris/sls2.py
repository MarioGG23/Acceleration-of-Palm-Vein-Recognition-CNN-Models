...
def sistema_lineal_sobredeterminista(X, y):
    ...
    X1_transpose = X1.T  
    product = X1_transpose.dot(X1)
    inverse_product = np.linalg.inv(product)
    pseudoinversa = inverse_product.dot(X1_transpose)
    ...
    y_val_pred = (X_val.dot(pseudoinversa) >= 0.5).astype(int)
    val_accuracy = accuracy_score(y_val, y_val_pred)

    

    X_test = np.array(X_test)
    y_test_pred = (X_test.dot(pseudoinversa) >= 0.5).astype(int)
